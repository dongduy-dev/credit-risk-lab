...
So sanh cac do do: accuracy, precision, recall, f1-score (tung class + weighted avg),
...
def main():
    ...
    #    stratify=y -> giu dung ty le class 0/1 o ca train lan test,
    #    quan trong voi du lieu mat can bang de metric khong bi lech.
    ...
    for name, estimator in get_models().items():
        ...
        print(f"{name:22s} | acc={acc:.4f} | weighted_f1={wf1:.4f} "
              f"| train={train_time:.2f}s | test={test_time:.3f}s")

        # chon model tot nhat theo weighted f1 de dung cho app demo
        if wf1 > best_f1:
            best_f1, best_name, best_pipeline = wf1, name, pipe

    # 3) Luu ket qua so sanh + model tot nhat
    payload = {"best_model": best_name, "results": results}
    ...
    print(f"\nBest model: {best_name} (weighted_f1={best_f1:.4f})")

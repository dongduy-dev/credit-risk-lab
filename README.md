...
Đồ án giữa kỳ môn Nhập môn Học máy (Machine Learning), sử dụng bộ dữ liệu UCI **Default of Credit Card Clients** (Dataset ID 350). Mục tiêu: dự đoán liệu một khách hàng thẻ tín dụng có vỡ nợ trong kỳ thanh toán tiếp theo hay không.
...
## Mức độ hoàn thành yêu cầu đồ án

| Yêu cầu | Trạng thái | Minh chứng |
|---|---|---|
| Đọc dataset UCI | Hoàn thành | `data/credit_card_default.csv`; giữ lại file XLS gốc |
| Preprocessing & normalize | Hoàn thành | `ColumnTransformer` với `StandardScaler` và `OneHotEncoder` |
| Dùng ≥ 3 classifier | Hoàn thành | Logistic Regression, Random Forest, Gradient Boosting |
| Báo cáo per-class metrics | Hoàn thành | `artifacts/model_comparison.json`, trang Model Comparison |
| Báo cáo thời gian train/test | Hoàn thành | `train_time_sec`, `test_time_sec` trong model artifact |
| So sánh classification có ý nghĩa | Hoàn thành | Baseline, default recall/F1, false positive/negative, phân tích rủi ro |
| Feature selection bằng correlation | Hoàn thành | `r_regression` ranking + `SelectKBest(f_regression)` trên tập train |
| Trực quan hóa correlation | Hoàn thành | Bar chart ranking + heatmap trong Streamlit |
| Nhiều feature subset | Hoàn thành | top_5, top_10, top_15, all |
| Linear Regression + MAE | Hoàn thành | `src/feature_selection.py` và trang Feature Selection |
| Demo Streamlit | Hoàn thành | `app.py` với các trang prediction, metrics, feature selection, statistics, team info |
